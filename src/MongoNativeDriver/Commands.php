...
trait Commands {    
    ...
    protected function findNormal(array $argv = null ) { 
        $paginate_options = '' ; 
    ...
    protected function findGroup (array $argv = null ) {  
        $paginate_options = '' ; 
        $paginate_perpage = null ; 
        $query_random  = null ; 
    ...
    protected function findJoin(array $argv=null ) { 
        $paginate_options = '' ; 
        $paginate_perpage = null ; 
        $query_random  = null ; 
    ...
    public static function getSequence(String $autoIncName , String $collection = null ) { 
        ...
        // isin_schema($autoIncName)?:throw new Exception("collection '$autoIncName' aren't in member schema collection $collection  ") ;
        if(!(new static)->isfield_incollection($collection ,$autoIncName ))throw new Exception("Fail getSequence() for field name  $autoIncName is not be member collection $collection ");
        $config = new Config;
    ...
    protected static function InitIndexAutoInc () { 
        $this->fillable( (array) [] );
    }

    private function isfield_incollection(string $collection , string $autoIncName  ){
        if( $this->isin_schema($collection ) ){ 
            $allField = [];
            foreach( $this->schema[$collection] as $key => $value ){
              if(is_array($value)){ 
                  array_push($allField,$key);
                }else{
                  array_push($allField,$value);
                }; 
            };
            if(in_array($autoIncName,$allField)==true){return true ;}else{ return false;}
        }else{
            throw new Exception("Error  collection $collection aren't be member in \$schema ");
            return false ; 
        };
    }
    ...
    *
    */
    protected static function pagedrawing($perpage, $totaldocument, $request_page_number) {
